...
static VkPhysicalDevice SelectPhysicalDevice(VkInstance instance) {
  ...
  auto result = vkEnumeratePhysicalDevices(instance, &physicalDeviceCount,
...
class DeviceManager {
 public:
  ...
  DeviceManager(const VkPhysicalDevice& physicalDevice,
...
#undef CreateSemaphore
  ...
  VkImageView CreateColorImageView2D(
      VkImage image, VkFormat format = VK_FORMAT_R8G8B8A8_SRGB) {
    ...
    imageViews[imageView] =
        VkImageViewUnique(imageView, VkImageViewDeleter(device));

    return imageView;
  }

  void DestroyImageView(const VkImageView& view) { imageViews.erase(view); }

  VkFramebuffer CreateFramebuffer(const VkRenderPass& renderPass,
    ...
    framebuffers[framebuffer] =
        VkFramebufferUnique(framebuffer, VkFramebufferDeleter(device));
  ...
  VkCommandPool CreateCommandPool(const uint32_t& queueFamilyIndex,
    ...
    createInfo.flags |=
        poolReset ? VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT : 0;
    createInfo.queueFamilyIndex = queueFamilyIndex;
    vkCreateCommandPool(GetDevice(), &createInfo, nullptr, &pool);
    commandPools[pool] =
        VkCommandPoolUnique(pool, VkCommandPoolDeleter(GetDevice()));
    return pool;
  }

  std::vector<VkCommandBuffer> AllocateCommandBuffers(const VkCommandPool& pool,
  ...
  VkRenderPass CreateRenderPass(VkSurfaceFormatKHR& surfaceFormat,
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface, &formatCount,
                                         formats.data());
    // TODO: possibly pass this in so that the application can choose the
    // defaults
    surfaceFormat = SelectSurfaceFormat(formats, VK_FORMAT_R8G8B8A8_UNORM,
    ...
    for (const auto& subpassJson : renderPassConfig["subpasses"]) {
      ...
      for (const auto& colorAttachmentJson : subpassJson["colorAttachments"]) {
        auto& colorAttachment =
            subpassData.colorAttachments.emplace_back(VkAttachmentReference());
        colorAttachment.attachment = colorAttachmentJson["attachment"];
        colorAttachment.layout = colorAttachmentJson["layout"];
      }
      subpass.colorAttachmentCount =
          gsl::narrow<uint32_t>(subpassData.colorAttachments.size());
      ...
      if (!depthStencilAttachmentJson.is_null()) {
        ...
        subpassData.depthStencilAttachment.value().attachment =
            depthStencilAttachmentJson["attachment"];
        subpassData.depthStencilAttachment.value().layout =
            depthStencilAttachmentJson["layout"];
      }
      if (subpassData.depthStencilAttachment.has_value()) {
        subpass.pDepthStencilAttachment =
            &subpassData.depthStencilAttachment.value();
      } else {
        subpass.pDepthStencilAttachment = nullptr;
      }

      for (const auto& inputAttachmentJson : subpassJson["inputAttachments"]) {
        auto& inputAttachment =
            subpassData.inputAttachments.emplace_back(VkAttachmentReference());
        inputAttachment.attachment = inputAttachmentJson["attachment"];
        inputAttachment.layout = inputAttachmentJson["layout"];
      }
      subpass.inputAttachmentCount =
          gsl::narrow<uint32_t>(subpassData.inputAttachments.size());
      subpass.pInputAttachments = subpassData.inputAttachments.data();

      for (const auto& resolveAttachmentJson :
           subpassJson["resolveAttachments"]) {
      ...
      for (const auto& preserveAttachmentJson :
           subpassJson["preserveAttachments"]) {
        subpassData.preserveAttachments.push_back(preserveAttachmentJson);
      }
      subpass.preserveAttachmentCount =
          gsl::narrow<uint32_t>(subpassData.preserveAttachments.size());
    ...
    createInfo.attachmentCount =
        gsl::narrow<uint32_t>(attachmentDescriptions.size());
    createInfo.pAttachments = attachmentDescriptions.data();
    createInfo.subpassCount = gsl::narrow<uint32_t>(subpassDescriptions.size());
    createInfo.pSubpasses = subpassDescriptions.data();
    createInfo.dependencyCount =
        gsl::narrow<uint32_t>(subpassDependencies.size());
    ...
    renderPasses[renderPass] =
        VkRenderPassUnique(renderPass, VkRenderPassDeleter(device));
 ...
 private:
  ...
  ::VkPresentModeKHR SelectPresentMode(
    ...
    auto testPreferredSupport =
        std::find(supported.begin(), supported.end(), preferred);
 ...
 public:
  VkSwapchainKHR CreateSwapchain(const VkSurfaceKHR& surface,
    ...
    auto selectedPresentMode =
        SelectPresentMode(presentModes, swapchainConfig["presentMode"]);
    ...
    auto preferredCompositeAlpha =
        swapchainConfig["compositeAlpha"].get<VkCompositeAlphaFlagsKHR>();
    if (capabilities.supportedCompositeAlpha & preferredCompositeAlpha) {
      createInfo.compositeAlpha =
          static_cast<VkCompositeAlphaFlagBitsKHR>(preferredCompositeAlpha);
    } else {
    ...
    swapchains[swapchain] =
        VkSwapchainKHRUnique(swapchain, VkSwapchainKHRDeleter(device));
  ...
  VkShaderModule CreateShaderModule(json shaderJson) {
    ...
    shaderModules[shaderModule] =
        VkShaderModuleUnique(shaderModule, VkShaderModuleDeleter(device));

    return shaderModule;
  }

  VkSampler CreateSampler(json samplerJson) {
  ...
  VkDescriptorSetLayout CreateDescriptorSetLayout(
    ...
    descriptorSetLayouts[setLayout] = VkDescriptorSetLayoutUnique(
        setLayout, VkDescriptorSetLayoutDeleter(device));

    return setLayout;
  }

  VkDescriptorPool CreateDescriptorPool(json descriptorSetLayoutJson,
    ...
    descriptorPools[pool] =
        VkDescriptorPoolUnique(pool, VkDescriptorPoolDeleter(device));
  ...
  VkPipelineLayout CreatePipelineLayout(
    ...
    createInfo.pushConstantRangeCount =
        gsl::narrow<uint32_t>(pushConstantRanges.size());
    ...
    pipelineLayouts[pipelineLayout] =
        VkPipelineLayoutUnique(pipelineLayout, VkPipelineLayoutDeleter(device));
  ...
  VkPipeline CreateGraphicsPipeline(
    ...
    for (auto i = 0U; i < 4; ++i) {
      colorBlendState.blendConstants[i] =
          colorBlendConfigJson["blendConstants"][i];
    }
    std::vector<VkPipelineColorBlendAttachmentState> colorBlendAttachments;
    for (const auto& colorBlendJson :
         colorBlendConfigJson["colorBlendAttachments"]) {
      ...
      colorBlendAttachment.srcColorBlendFactor =
          colorBlendJson["srcColorBlendFactor"];
      colorBlendAttachment.dstColorBlendFactor =
          colorBlendJson["dstColorBlendFactor"];
      colorBlendAttachment.alphaBlendOp = colorBlendJson["alphaBlendOp"];
      colorBlendAttachment.srcAlphaBlendFactor =
          colorBlendJson["srcAlphaBlendFactor"];
      colorBlendAttachment.dstAlphaBlendFactor =
          colorBlendJson["dstAlphaBlendFactor"];
    ...
    colorBlendState.attachmentCount =
        gsl::narrow<uint32_t>(colorBlendAttachments.size());
    ...
    depthStencilState.depthBoundsTestEnable =
        depthStencilJson["depthBoundsTestEnable"];
    ...
    std::map<VkShaderModule, std::vector<VkSpecializationMapEntry>>
        shaderSpecializationMapEntries;
    for (const auto& shaderStageJson : shaderStagesJson) {
      ...
      for (const auto& mapEntryJson :
           shaderStageJson["specializationMapEntries"]) {
      ...
      specInfo.mapEntryCount = gsl::narrow<uint32_t>(
          shaderSpecializationMapEntries[shaderModule].size());
      specInfo.pMapEntries =
          shaderSpecializationMapEntries[shaderModule].data();
      ...
      shaderStage.pSpecializationInfo =
          &shaderSpecializationInfos[shaderModule];
    ...
    inputAssemblyState.primitiveRestartEnable =
        inputAssemblyJson["primitiveRestartEnable"];
    ...
    multisampleState.sampleShadingEnable =
        multisampleJson["sampleShadingEnable"];
    multisampleState.minSampleShading = multisampleJson["minSampleShading"];
    multisampleState.rasterizationSamples =
        multisampleJson["rasterizationSamples"];
    multisampleState.alphaToCoverageEnable =
        multisampleJson["alphaToCoverageEnable"];
    ...
    rasterizationState.rasterizerDiscardEnable =
        rasterizationJson["rasterizerDiscardEnable"];
    ...
    for (const auto& inputAttrDescJson :
         vertexInputJson["vertexAttributeDescriptions"]) {
      auto& attributeDescription = vertexAttributeDescriptions.emplace_back(
    ...
    for (const auto& inputBindingDescJson :
         vertexInputJson["vertexBindingDescriptions"]) {
      auto& bindingDescription = vertexBindingDescriptions.emplace_back(
    ...
    vertexInputState.vertexAttributeDescriptionCount =
        gsl::narrow<uint32_t>(vertexAttributeDescriptions.size());
    vertexInputState.pVertexAttributeDescriptions =
        vertexAttributeDescriptions.data();
    vertexInputState.vertexBindingDescriptionCount =
        gsl::narrow<uint32_t>(vertexBindingDescriptions.size());
    ...
    VkPipeline pipeline;
    vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &createInfo, nullptr,
                              &pipeline);
 ...
 private:
  ...
  std::map<VkDescriptorSetLayout, VkDescriptorSetLayoutUnique>
  ...
  void GetQueueFamilyProperties() {
    uint32_t propCount;
    vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &propCount,
                                             nullptr);
  ...
  void CreateDevice() {
    ...
    createInfo.queueCreateInfoCount =
        gsl::narrow<uint32_t>(queueCreateInfos.size());
    createInfo.pQueueCreateInfos = queueCreateInfos.data();
    createInfo.enabledLayerCount = 0;
    createInfo.ppEnabledLayerNames = nullptr;
    createInfo.enabledExtensionCount =
        gsl::narrow<uint32_t>(deviceExtensions.size());
