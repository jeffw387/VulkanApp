 ...
 public:
  ...
  QuadTreeFixture() {
    ...
    for (auto& obj : objVector) {
      ...
      auto pos = static_cast<double>((time - startTime).count() %
                                     static_cast<uint64_t>(radius * 2.0));
      pos -= radius;
...
// success depends on choosing a small enough retrieval region to not request
// the entire tree the bit math to select the retrieval area rounds up, causing
