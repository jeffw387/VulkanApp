...
struct Rect {
  ...
  double GetRight() const { return x + xRadius; }

  double GetTop() const { return y - yRadius; }

  double GetBottom() const { return y + yRadius; }
...
class Tree {
 public:
  ...
  class RegionIterator {
   public:
    ...
    bool PastEnd() { return current == nullptr; }
    ...
    for (auto depth = 0U; depth < 7U; ++depth) {
      ...
      for (auto y = 0U; y < cellsPerAxis; y++) {
        for (auto x = 0U; x < cellsPerAxis; ++x) {
          ...
          node.SetRegion(Rect((2 * x * radiusAtDepth) - radius + radiusAtDepth,
 ...
 private:
  static uint8_t HighestBit(const uint8_t& bits) {
    ...
    if (bits & Bit7) return static_cast<uint8_t>(0);
    if (bits & Bit6) return static_cast<uint8_t>(1);
    if (bits & Bit5) return static_cast<uint8_t>(2);
    if (bits & Bit4) return static_cast<uint8_t>(3);
    if (bits & Bit3) return static_cast<uint8_t>(4);
    if (bits & Bit2) return static_cast<uint8_t>(5);
    if (bits & Bit1) return static_cast<uint8_t>(6);
