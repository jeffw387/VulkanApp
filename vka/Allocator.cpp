...
static bool operator!=(const AllocationHandle& lhs,
                       const AllocationHandle& rhs) {
  return (lhs.memory != rhs.memory) || (lhs.size != rhs.size) ||
         (lhs.offsetInDeviceMemory != rhs.offsetInDeviceMemory) ||
         (lhs.typeID != rhs.typeID) || (lhs.block != rhs.block);
}
...
MemoryBlock::MemoryBlock(const VkDevice& device,
                         const VkMemoryAllocateInfo& allocateInfo)
    : device(device), allocateInfo(allocateInfo) {
...
// returns the offset of a allocation that can contain the requested allocation
// if possible
std::optional<VkDeviceSize> MemoryBlock::CanAllocate(
    const VkMemoryRequirements& requirements) {
  for (auto& allocPair : allocations) {
    if (allocPair.second.allocated) continue;
    if ((helper::roundUp(allocPair.first, requirements.alignment) +
         requirements.size) <= (allocPair.first + allocPair.second.size))
      return std::make_optional<VkDeviceSize>(allocPair.first);
  }
  return std::make_optional<VkDeviceSize>();
}

// divides allocation at the given offset, returning the offset of the requested
// allocation this does not bounds check, CanAllocate() should have been called
// first
VkDeviceSize MemoryBlock::DivideAllocation(
    const VkDeviceSize firstOffset, const VkMemoryRequirements& requirements) {
  ...
  // set second allocation's size (end of original allocation minus beginning of
  // new allocation)
...
AllocationHandle MemoryBlock::CreateHandleFromAllocation(
...
std::optional<uint32_t> Allocator::ChooseMemoryType(
    VkMemoryPropertyFlags memoryFlags,
    const VkMemoryRequirements& requirements) {
  for (auto i = 0U; i < memoryProperties.memoryTypeCount; i++) {
    if (((1 << i) & requirements.memoryTypeBits) &&
        ((memoryProperties.memoryTypes[i].propertyFlags & memoryFlags) ==
         memoryFlags)) {
...
MemoryBlock& Allocator::AllocateNewBlock(
    const VkMemoryAllocateInfo& allocateInfo) {
...
AllocationHandle Allocator::AllocateMemory(
    const bool DedicatedAllocation, const VkMemoryRequirements& requirements,
  ...
  // if a dedicated allocation is not required, attempt to allocate from
  // existing blocks
  if (!DedicatedAllocation) {
    ...
    for (auto& block : memoryBlocks) {
      if (block.allocateInfo.memoryTypeIndex != typeID.value()) continue;
      auto optionalOffset = block.CanAllocate(requirements);
      if (!optionalOffset) continue;
  ...
  if (!DedicatedAllocation) {
    allocateInfo.allocationSize =
        std::max<VkDeviceSize>(allocateInfo.allocationSize, defaultBlockSize);
  }
...
AllocationHandle Allocator::AllocateForImage(
    const bool DedicatedAllocation, const VkImage image,
    VkMemoryPropertyFlags memoryFlags) {
...
AllocationHandle Allocator::AllocateForBuffer(
    const bool DedicatedAllocation, const VkBuffer buffer,
    VkMemoryPropertyFlags memoryFlags) {
  VkMemoryRequirements requirements = {};
