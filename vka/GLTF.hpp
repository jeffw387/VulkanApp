	...
	namespace detail
	{
			...
			const BufferVector &buffers)
		{
			...
			if (componentType == 5120)
			{
				...
				for (auto currentComponent = 0U; currentComponent < elementCount * elementComponentCount; ++currentComponent)
				{
			...
			const BufferVector &buffers)
		{
			...
			CopyFromBufferView<glm::float32>(mesh.positions, positionAccessor, j, buffers);
