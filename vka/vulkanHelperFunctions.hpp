...
inline auto CreateAllocatedBuffer(
  ...
  vkCreateBuffer(device, &bufferCreateInfo, nullptr, &allocatedBuffer.buffer);

  allocatedBuffer.usage = usage;
  allocatedBuffer.size = size;
  allocatedBuffer.allocation = allocator.AllocateForBuffer(
...
inline auto BeginTransientCommandBuffer(VkCommandBuffer commandBuffer) {
...
inline auto CreateAllocatedImage2D(
  ...
  image2D.view = CreateImageView2D(device, image2D.image, format, aspects);
...
inline auto DestroyAllocatedImage2D(VkDevice device, Image2D image2D) {
...
inline auto AllocateDescriptorSets(
...
inline auto AllocateCommandBuffers(
...
inline auto CreateGraphicsPipelines(
